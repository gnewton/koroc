...
func readFromFileAndExtractXML(c chan string, dbc *DBConnector, articleChannel chan []*Article, wg *sync.WaitGroup) {
	defer wg.Done()
...
func pubmedArticleToDbArticle(p *pubmedstruct.PubmedArticle) *Article {
	...
	dbArticle.pmidString = p.MedlineCitation.PMID.Text
